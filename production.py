...
try:
    ...
    from sets import Set as set, ImmutableSet as frozenset # type: ignore
        ...
        if bindings != None:
            ...
            if isinstance(rule.antecedent(),(OR, AND)):
                for antecedent in rule.antecedent():
                    new_hypo = instantiate(antecedent, bindings)
                    subtree.append(backward_chain(rules, new_hypo))
                    goal_tree.append(subtree)
            else:
                new_hypo = instantiate(rule.antecedent(), bindings)
                subtree.append(backward_chain(rules, new_hypo))
                goal_tree.append(subtree)
...
class IF(object):
    ...
    def __init__(self, conditional, action = None, 
        ...
        if type(conditional) == list and action == None:
            return apply(self.__init__, conditional) # type: ignore
